document.addEventListener("DOMContentLoaded", async () => {
  ...
  const autoDetectStatusText = document.getElementById("autoDetectStatusText");
    ...
    chrome.storage.local.get([domainKey, segmentationKey, autoDetectKey, dictionaryKey, masterKey], (result) => {
      ...
      const useAutoDetect =
        typeof result[autoDetectKey] === "boolean" ? result[autoDetectKey] : true; // default true
      autoDetectToggle.checked = useAutoDetect;
      updateAutoDetectStatusText(useAutoDetect);

      const useDictionary =
        typeof result[dictionaryKey] === "boolean" ? result[dictionaryKey] : true; // default true
